...
* El **.RUred[aspecto gramatical]** se refiere a *cómo* un evento avanza por un tiempo
...
--
```{r, load-lex-aspect-diagram}
knitr::include_graphics("./Diagrams/Lexical Aspect (Spanish).png")
```
...
--
- No hemos identificado cómo un hablante usa el pretérito con algunos estados, pero no otros
...
--
  + Cantidades de uso de la lengua de herencia afectan la *reestructuración*

--
  + La producción afectada antes de la comprensión

--
  + *Reensamblaje* de rasgos lingüísticos (e.g., Lardiere, 2008): viv.RUred[í] → cre.RUred[í], com.RUred[í], pero no hab.grey[í]
...
---
# Efectos de regularidad morfológica
```{r, preterit-morph-regularity}
Preterit_by_Morph_Regularity <- Preterit_Aggregate %>%
  ggplot(aes(x = Reg_Main, y = ExpForm, color = Grupo, shape = Tarea)) + 
  ...
  stat_summary(fun.data = mean_se,
               ...
               position = position_dodge(width = 0.5)) + 
  scale_color_brewer(palette = "Set1", name = "") +
  labs(x = "Regularidad morfológica", y = 'Proporción de uso del pretérito', 
       title = "Uso del pretérito según la regularidad morfológica") +
  theme(axis.text = element_text(size = 16),
        axis.title = element_text(size = 18, face = "bold"),
        plot.title = element_text(hjust = 0.5, size = 24, face = "bold"))
```

```{r, publish-morph-reg-graph, height = 10, fig.width = 16, fig.align = 'center', fig.retina = 2}
(Preterit_by_Morph_Regularity)
```
...
```{r, plot-FCT-model}
FCT_Plot <- plot_model(Aspect_Comprehension, show.values = TRUE, value.offset = .3, transform = NULL, vline.color = "red") +
  scale_x_discrete(labels = c("Frecuencia : regularidad : uso", "Regularidad : uso", "Frecuencia : uso", "Frecuencia : regularidad", "Edad de adquisición", "Frecuencia de uso", "Regularidad morfológica", "Frecuencia léxica")) + 
...
---
# Pretérito en producción por uso del español y edad
```{r, preterit-FofA}
...
.small[Andersen, R. (1991). Developmental sequences: The emergence of aspect marking in second language acquisition. In T. Huebner & C. A. Ferguson (Eds.), *Crosscurrents in second language acquisition and linguistic theories* (pp. 305-324). Amsterdam: John Benjamins.]
