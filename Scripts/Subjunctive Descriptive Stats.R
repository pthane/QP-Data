...
Heritage_EPT_Standardized = aggregate(Heritage_EPT$Response, list(Heritage_EPT$Participant_ID), FUN = mean, na.rm = TRUE)
...
Heritage_FCT_Standardized = aggregate(Heritage_FCT$Response, list(Heritage_FCT$Participant_ID), FUN = mean, na.rm = TRUE)
...
Subjunctive_EPT_Aggregate = aggregate(Subjunctive_EPT$Response, list(Subjunctive_EPT$Participant_ID), FUN = mean, na.rm = TRUE)
...
Subjunctive_FCT_Aggregate = aggregate(Subjunctive_FCT$Response, list(Subjunctive_FCT$Participant_ID), FUN = mean, na.rm = TRUE)
...
Master_Subjunctive_Aggregate = rbind(Subjunctive_EPT_Aggregate, Subjunctive_FCT_Aggregate)
...
Subjunctive_Aggregate %>%
  ...
  labs(x = "Matrix verb (from highest to lowest frequency)", y = "Proportion of subjunctive responses", color = "Group",
       title = "Average Use of Subjunctive by Matrix Verb") +
...
Master_Subjunctive_Aggregate %>% 
  ...
  scale_color_manual(values = c("#F8766D", "#619CFF", "#00BA38")) +
  labs(x = "Correct responses on DELE proficiency measure", y = "Proportion of subjunctive responses", caption = "", 
title = "Subjunctive by Proficiency", color = "Group", shape = "Task") +
  theme(plot.title = element_text(hjust = 0.5))
...
Master_Subjunctive_Aggregate %>%
  ...
  scale_color_manual(values = c("#F8766D", "#619CFF", "#00BA38")) +
  labs(x = "Self-reported frequency of use of Spanish", y = "Proportion of subjunctive responses",
       title = "Subjunctive by Frequency of Use", color = "Group", shape = "Task") +
  theme(plot.title = element_text(hjust = 0.5))
...
Heritage_Subjunctive_Aggregate %>%
  ...
  labs(x = "Age of onset of acquisition of English", y = "Proportion of subjunctive responses", caption = "", 
       title = "Subjunctive by Age of Acquisition") +
  theme(plot.title = element_text(hjust = 0.5))
